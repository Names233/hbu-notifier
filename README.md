...
每条通知推送时，程序会抓取详情页正文，交给 AI 做两件事：**概括成一句话**（默认不超过 60 字），以及**抽取开始/截止时间**（通知里若写明选课、报名的起止时间）。推送里会显示出来，并自动算好倒计时：

```
**摘要**：2022级五年制及2023—2026级在籍本科生须于9月20日12:00至9月22日18:00登录教务系统完成电子注册。
**🕐 开始 09-20 12:00（已开始）**
**⏰ 截止 09-22 18:00（还剩 2 天）**
```

- **模型**：走 freeshare 中转（`https://freeshare.cc.cd/v1`，OpenAI 兼容）。主模型 `deepseek-v4.1-flash`，失败自动回退 `kimi-k3` → `glm-5.3-flash`。该中转站偶发限流/空回复，回退链就是为此设计的。
- **防编造**：提示词里明确要求"只填通知中明确写出、或据其规定完全可以确定的时间，禁止推测"，且只写月日时会以通知发布日期的年份补齐。
- **密钥**：需要在仓库 Secrets 里配 `FREESHARE_API_KEY`（就是那个 `sk-` 开头的 key）。**这是 AI 功能能用的前提**，没配的话整条推送仍会正常发出，只是不带摘要与时间。
- **降级原则**：AI 或抓正文任何一步失败，程序绝不会漏推或卡住，只是这条推送少几行摘要/时间；模型没按 JSON 格式返回时，也会退化为"只有摘要、没有时间"而不是整条丢失。摘要与时间都存进 `state.json`，供汇总复用。
- **换模型 / 关掉**：改 `config.toml` 的 `[ai]` 段。`enabled = false` 即完全关闭；换 `base_url` + `api_key` + `model` 即可切到任意 OpenAI 兼容服务。
